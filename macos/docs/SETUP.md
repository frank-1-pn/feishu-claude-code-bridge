...
```
Monitor({ command: "~/.claude/bin/feishu/feishu-tail.sh <名字>",
          description: "飞书事件桥接 <名字>", persistent: true, timeout_ms: 3600000 })
```

`feishu-tail.sh` = 裸 `tail -n 0 -F` 外加两层防护(仍只是只读 tail,绝不碰订阅连接):

- **message_id 去重**:启动时把现有日志里的 id 全 seed 进表,之后只放行从未见过的 id。
  根治"**日志被重写/截断后 `tail -F` 从头重放历史**"造成的【重复消息刷屏】——launchd 长连接定期
  重连会重写日志,裸 tail 会把已处理过的近期消息当新事件再喷一遍(用户看到一直说"duplicate")。
- **卡键乱码过滤**:丢掉手机长按重复发的 `A#` / `A #` / `A\` 等 ≤5 字符短垃圾(刷屏量大能把
  Monitor 冲到 auto-stop → 重 arm 用 `-n 0` 又跳过积压 → 真漏消息)。
- 仍 `-n 0`:arm 时不回放历史。没 arm 这层 → 消息进日志但 Claude 收不到(用户感觉"石沉大海")。
- (也可直接裸 `tail -n 0 -F ~/Library/Logs/feishu/<名字>.log`,但遇到上面两种情况会刷屏/漏消息。)
...
2. 解析到某 bot → arm Monitor: `~/.claude/bin/feishu/feishu-tail.sh <bot>`(persistent;过滤+去重,免重放刷屏)
